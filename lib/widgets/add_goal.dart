...
import 'package:flutter_financemanager/widgets/add_goal_select_category.dart';
...
class _AddGoalState extends State<AddGoal> {
  ...
  void onClickConfirmButton() async {
    if (selectedCategory.isNotEmpty && amountController.text.isNotEmpty) {
      ...
      // 목표 추가하는 api사용
      final addGoalResult = await GoalService.addGoalService(
        categoryToUpperMap[selectedCategory]!,
        formattedDate,
        amount,
      );
      Navigator.of(context).pop(addGoalResult);
    }
  }
        ...
        onPress: () async {
          var result = await showDialog(
            ...
            builder: (BuildContext context) {
              return const AddGoalSelectCategory();
            },
          );
          if (result != null) {
            setState(() {
